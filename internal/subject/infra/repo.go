...
func (r *PostgresRepository) Create(ctx context.Context, s domain.Subject) error {
	_, err := r.pool.Exec(ctx,
		`INSERT INTO subjects (id, name, detail, owning_organization_id, created_by_account_id, created_at)
		 VALUES ($1, $2, $3, $4, $5, $6)`,
		s.ID, s.Name, s.Detail, s.OwningOrganizationID, s.CreatedByAccountID, s.CreatedAt)
	if err != nil {
...
func (r *PostgresRepository) FindByID(ctx context.Context, id string) (domain.Subject, error) {
	...
	err := r.pool.QueryRow(ctx,
		`SELECT id, name, detail, owning_organization_id, created_by_account_id, created_at
		 FROM subjects WHERE id = $1`, id).
		Scan(&s.ID, &s.Name, &s.Detail, &s.OwningOrganizationID, &s.CreatedByAccountID, &s.CreatedAt)
	if err != nil {
...
func scanSubjects(rows pgx.Rows) ([]domain.Subject, error) {
	...
	for rows.Next() {
		var s domain.Subject
		if err := rows.Scan(&s.ID, &s.Name, &s.Detail, &s.OwningOrganizationID, &s.CreatedByAccountID, &s.CreatedAt); err != nil {
