...
var _ interface {
	...
	CreatePropertyOwner(ctx context.Context, dto CreatePropertyOwnerDTO) (string, error)
	InvitePropertyOwner(ctx context.Context, dto InvitePropertyOwnerDTO) error
} = (*facadeImpl)(nil)
...
func (f *facadeImpl) CreatePropertyOwner(ctx context.Context, dto CreatePropertyOwnerDTO) (string, error) {
	return f.parties.CreateParty(ctx, partyfacade.CreatePartyDTO{
		Name:               dto.Name,
		ActorKind:          partyfacade.ActorKindHuman,
		PartyKind:          partyfacade.PartyKindPropertyOwner,
		OwningOrgID:        dto.OrgID,
		CreatedByAccountID: dto.CreatedByAccountID,
	})
}

func (f *facadeImpl) InvitePropertyOwner(ctx context.Context, dto InvitePropertyOwnerDTO) error {
	inv, err := f.invitations.CreateInvitationAsActor(ctx, dto.ActorAccountID, dto.OrgID, dto.Email)
	if err != nil {
		return fmt.Errorf("create invitation for property owner: %w", err)
	}

	_, err = f.accounts.CreatePendingPartyLink(ctx, inv.ID, dto.PropertyOwnerPartyID, dto.OrgID)
	if err != nil {
		return fmt.Errorf("create pending party link for property owner invitation: %w", err)
	}
